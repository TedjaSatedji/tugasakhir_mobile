...
class _EditTransactionScreenState extends State<EditTransactionScreen> {
  ...
  void initState() {
    ...
    _selectedCategory = widget.transaction.category ?? ExpenseCategory.food;
    _selectedDate = widget.transaction.timestamp;
    _existingReceiptUrl = widget.transaction.receiptImageUrl;
    ...
    final formattedAmount = NumberFormat.decimalPattern('en_US')
        .format(widget.transaction.amount.round());
          ...
          children: [
            ...
            Row(
              children: [
                ...
                if (_receiptImagePath != null || _existingReceiptUrl != null) ...[
                  const SizedBox(width: 10),
                  IconButton(
                    onPressed: () {
                      setState(() {
  ...
  Future<void> _saveEdits() async {
    ...
    String? finalImageUrl = _existingReceiptUrl;
    if (_receiptImagePath != null && _receiptImagePath!.isNotEmpty) {
      finalImageUrl = _receiptImagePath;
      final uploadedUrl = await SyncService().uploadImage(_receiptImagePath!);
      if (uploadedUrl != null) {
    ...
    final xpToRemove = widget.transaction.xpAwarded > 0
        ? widget.transaction.xpAwarded
        : (widget.transaction.amount / 1000).floor();

    if (widget.transaction.missionCompletedId != null &&
        widget.transaction.missionCompletedDateKey != null) {
      await questProvider.revokeDailyMission(
        widget.transaction.missionCompletedId!,
        widget.transaction.missionCompletedDateKey!,
      );
    }
    ...
    if (widget.transaction.coinsAwarded > 0) {
      await context
          .read<CharacterProvider>()
          .removeCoins(widget.transaction.coinsAwarded);
    }
